...
plt.style.use("dark_background")               # global dark style
...
# ── add text labels -----------------------------------------------------------
for i, (rect, label, fs) in enumerate(zip(ax.patches, labels, font_sizes)):
    x, y, w, h = rect.get_x(), rect.get_y(), rect.get_width(), rect.get_height()

    # glow / shadow effect (optional)
    ax.text(
        x + w / 2, y + h / 2 + 0.002,
    ...
    ax.text(
        ...
        color="#ffffff", family="DejaVu Sans",
    )

# ── legend for color codes ----------------------------------------------------
legend_elements = [
    mpatches.Patch(color="#006400", label="≥ +5 %"),
    mpatches.Patch(color="#2ca02c", label="+0 % to +4.99 %"),
    mpatches.Patch(color="#d62728", label="-0.01 % to -4.99 %"),
    mpatches.Patch(color="#8B0000", label="≤ -5 %"),
]
ax.legend(
...
# ── titles & save -------------------------------------------------------------
title = f"S&P 500 — Top 100 by Market Cap | Daily Price Change as of {td}"
ax.set_title(title,
             fontsize=20,
             fontweight="bold",
             pad=20,
...
plt.savefig("./snp500_top100_treemap.png",
            dpi=300,
            bbox_inches="tight",
