...
class MADC(Stock):
    ...
    def plot_chart_madc_html(self):
        ...
        # plot the ma_diff
        #plt.bar(ma_diff.index, ma_diff.values, width=0.5, align='center', label=(f"MA{self.short_ma} - MA{self.long_ma}"), color='gray')
...
class FibonacciRetracement(Stock):
    ...
    def calculate_swing(self, ticker_data):
        ...
        for i in range(1, ticker_data.shape[0]-1):
            #if ticker_data['High'].iloc[i].values > ticker_data['High'].iloc[i-1].values and ticker_data['High'].iloc[i].values > ticker_data['High'].iloc[i+1].values and (highest_swing == -1 or ticker_data['High'].iloc[i].values > ticker_data['High'][highest_swing]):
            if (ticker_data['High'].iloc[i].values > ticker_data['High'].iloc[i-1].values and
                ticker_data['High'].iloc[i].values > ticker_data['High'].iloc[i+1].values and
                (highest_swing == -1 or ticker_data['High'].iloc[i].values > ticker_data['High'].iloc[highest_swing].values)):
                highest_swing = i
            if (ticker_data['Low'].iloc[i].values < ticker_data['Low'].iloc[i-1].values and
            ticker_data['Low'].iloc[i].values < ticker_data['Low'].iloc[i+1].values and
            (lowest_swing == -1 or ticker_data['Low'].iloc[i].values < ticker_data['Low'].iloc[lowest_swing].values)):
                lowest_swing = i
...
class OBV(Stock):
    ...
    def calculate_obv(self):
        ...
        for i in range(1, len(ticker_data)):
            if ticker_data['Close'].iloc[i].values > ticker_data['Close'].iloc[i-1].values:
                current_obv = prev_obv + ticker_data['Volume'].iloc[i].values
            elif ticker_data['Close'].iloc[i].values < ticker_data['Close'].iloc[i-1].values:
                current_obv = prev_obv - ticker_data['Volume'].iloc[i].values
            else:
...
class BullBearIndicator(Stock):
    ...
    def is_bullish_ma(self):
        ...
        last_price = self.stock_data['Close'].iloc[-1].values
    ...
    def is_bullish_bd(self):
        ...
        if sma_short.iloc[-1].values > sma_long.iloc[-1].values and sma_short.iloc[-2].values < sma_long.iloc[-2].values:
            return True
        else:
            return False
    ...
    def is_bullish_rsi(self):
        ...
        if rsi.iloc[-1].values < 30 and rsi.iloc[-2].values > 30:
            return True
        else:
            return False
    ...
    def is_bearish_rsi(self):
        rsi = self.calculate_rsi(self.tf_rsi)
        if rsi.iloc[-1].values > 70 and rsi.iloc[-2].values < 70:
            return True
        else:
            return False
    ...
    def is_bullish_bollinger_bands(self):
        ...
        if self.stock_data['Close'].iloc[-1].values < self.stock_data['Lower Band'].iloc[-1]:
            return True
        else:
            return False
    ...
    def is_bearish_bollinger_bands(self):
        ...
        if self.stock_data['Close'].iloc[-1].values > self.stock_data['Upper Band'].iloc[-1]:
            return True
        else:
            return False
