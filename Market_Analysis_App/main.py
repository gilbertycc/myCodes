...
class Stock:
    ...
    def get_price_target_date(self, target_date):
        try:
            ...
            ticker_data = yf.download(self.name_ticker, start=target_date, end=target_date_plus_one,
    ...
    def get_price_history(self):
        try:
            ticker_data = yf.download(self.name_ticker, period=self.data_period,
                                      progress=False, auto_adjust=True)
    ...
    def get_history_data(self):
        try:
            data_history = yf.download(self.name_ticker, period=self.data_period,
                                       progress=False, auto_adjust=True)
    ...
    def ml_RFR_html(self):
        ...
        html = f"<tr><td>{self.name_ticker}</td><td>{current_data.index[-1].strftime('%Y-%m-%d')}</td>" \
               f"<td>{model_name}</td><td>{round(float(r_pred[-1]), 2)}</td>" \
               f"<td>{round(float(current_data['Close'].values[-1]), 2)}</td></tr>"
        return html

    def __str__(self):
        return f"[***** Symbol Info *****]\nSymbol Type: {self.symbol_type}\n" \
               f"Ticker: {self.name_ticker}\nCurrent Data Period: {self.data_period}\n" \
               f"Latest Price: {self.get_current_price()}"
...
class MovingAverage(Stock):
    ...
    def plot_chart_MovingAverage_html(self):
        ...
        plt_html = f"<img src='data:image/png;base64,{plt_base64}' class='chart-img' " \
                   f"onclick='enlargeImage(this)' ondblclick='downsizeImage(this)'></a>"
        plt.close()
        return plt_html
...
class StochasticOscillator(Stock):
    ...
    def plot_chart_stochastic_oscillator(self):
        ...
        plt.figure(figsize=self.chart_figsize)
        plt.title(f"[DA] Stochastic Oscillator of symbol: {self.name_ticker} "
                 f"(Period: {self.data_period}, Timeframe: {self.timeframe} days)")
...
class RSI(Stock):
    ...
    def plot_chart_rsi(self):
        ...
        plt.figure(figsize=self.chart_figsize)
        plt.title(f"[DA] RSI Chart of symbol: {self.name_ticker} "
                 f"(Period: {self.data_period}, Timeframe: {self.timeframe} days)")
        plt.plot(data_history.index, data_history['Close'], label='Closing Price')
...
class MADC(Stock):
    ...
    def plot_chart_madc(self):
        ...
        plt.bar(ma_diff.index, ma_diff, width=0.5, align='center',
                label=(f"MA{self.short_ma} - MA{self.long_ma}"), color='gray')
        plt.legend(loc='upper left')
        plt.show()
...
class AccumulationDistributionLine(Stock):
    ...
    def calculate_adl(self):
        ...
        self.df['CMF Multiplier'] = ((self.df['Close'] - self.df['Low']) - (self.df['High'] - self.df['Close'])) / \
...
class RRnRRR(Stock):
    ...
    def get_rrr(self, entry_price, stop_loss, target_price):
        """
        Calculates the risk-reward ratio (RRR) for a given stock.
        Parameters:
            entry_price (float): The entry price of the stock.
            stop_loss (float): The stop loss price for the stock.
...
class BullBearIndicator(Stock):
    ...
    def is_bullish_bollinger_bands(self):
        ...
        return self.stock_data['Close'].iloc[-1] < self.stock_data['Lower Band'].iloc[-1]

    def is_bearish_bollinger_bands(self):
        self.calculate_bollinger_bands()
        return self.stock_data['Close'].iloc[-1] > self.stock_data['Upper Band'].iloc[-1]
...
def generate_html_body_stock(name_stock, review_stock, bb_signals_html_output, index_stock):
    html_output = f"""<div id="stock{index_stock}" class="tabcontent">
...
def generate_html_body_tab(name_stock, index_stock, index_end):
    if index_stock == 0:
        get_now_date_obj = datetime.now()
        today_date_stamp = get_now_date_obj.strftime("%b, %d %Y")
        html_output = f"""<h1>[DEMO] Stock Analysis Report</h1>
<p>Generated by Market_Analysis_App at {today_date_stamp} 
<a href="https://github.com/gilbertycc/myCodes/tree/main/Market_Analysis_App">Link to the Market Analysis App repository</a>
</p>
<div class="tab">
<button class="tablinks" onclick="openStock(event, 'stock{index_stock}')" id="defaultOpen">{name_stock}</button>
"""
    else:
        html_output = f"""<button class="tablinks" onclick="openStock(event, 'stock{index_stock}')">{name_stock}</button>
"""
...
def generate_html_end():
    html_output = """<div class="disclaimer">
<h3>Disclaimer</h3>
...
"""
        ...
        # Prepare the html report body stock
